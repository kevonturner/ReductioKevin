...
internal class Summarizer {
    ...
    func execute() -> [String] {
        buildGraph()
       return rank.execute().sorted(isOrderedBefore: { $0.1 > $1.1 }).map({ $0.0.text })
    }
    ...
    private func buildGraph() {
        ...
        combinations.forEach { combo in
            addNodes(pivotal: combo.first!, combo.last!)
        }
    }
    ...
    private func addNodes(pivotal: Sentence, _ node: Sentence) {
        ...
        rank.addEdge(from: pivotal, node, weight: score)
        rank.addEdge(from: node, pivotal, weight: score)
    }
}
